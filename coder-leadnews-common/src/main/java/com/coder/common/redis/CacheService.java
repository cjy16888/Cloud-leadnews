package com.coder.common.redis;
...
public class CacheService extends CachingConfigurerSupport {
    ...
    /**
     * 扫描主键，建议使用
     * scan的原理是游标分批获取，直到获取完毕，这样可以降低redis的内存消耗
     * 比如keys(*)命令，如果key数量非常多，会导致redis内存消耗过大，引起redis崩溃
     * 而scan则可以一次只获取少量的key，循环多次完成
     * @param patten
     * @return
     */
    public Set<String> scan(String patten){
        Set<String> keys = stringRedisTemplate.execute((RedisCallback<Set<String>>) connection -> {
            ...
            //count表示一次扫描的key数量，不要设置太大，否则会出现卡顿
    ...
    public String tryLock(String name, long expire) {
        name = name + "_lock";
        //随机生成一个value，作为redis的value
        String token = UUID.randomUUID().toString();
        //连接到我们的redis服务端
        RedisConnectionFactory factory = stringRedisTemplate.getConnectionFactory();
        //获取redis的连接，底层还是使用jedis的连接，因为LettuceConnection没有实现set方法
        //这是一个弊端，如果想使用Lettuce，那么就不能使用这种方式实现分布式锁
        RedisConnection conn = factory.getConnection();
        try {
            //参考redis命令：
            //set key value [EX seconds] [PX milliseconds] [NX|XX]
            //等同于：SET resource_name my_random_value NX PX 30000
            //NX：表示只有当key不存在时才set，也就是只有一个客户端能持有锁
